...
lv_res_t redBtnmAction(lv_obj_t *btnm, const char *txt){
	...
	autonToRun = pointerMap["r" + (std::string)btnmMap[autonIndex]];
...
lv_res_t blueBtnmAction(lv_obj_t *btnm, const char *txt)
{
	...
	color = blue;
	autonToRun = pointerMap["b" + (std::string)btnmMap[abs(autonIndex)]];
...
int tabWatcher() {
	...
	while(1){
		...
		if(currentTab != activeTab){
			...
			if(activeTab == 0){
				if(autonIndex == 0) autonIndex = 1;
				autonIndex = abs(autonIndex);
				lv_btnm_set_toggle(redBtnm, true, abs(autonIndex)-1);
			}else if(activeTab == 1){
				if(autonIndex == 0) autonIndex = -1;
				autonIndex = -abs(autonIndex);
				lv_btnm_set_toggle(blueBtnm, true, abs(autonIndex)-1);
			}else{
				autonIndex = 0;
			}
		}
...
void init(int hue, int default_auton, const char **autons){
	...
	lv_btnm_set_toggle(redBtnm, true, abs(autonIndex)-1);//3
